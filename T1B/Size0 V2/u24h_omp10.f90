...
do numLoops = 1, 4000
	...
	write(6,'(2x,a,i0)') 'Program "u24h_omp10.f90" uses no. of threads = ', Nthreads
	write(6,*) ' Inside parallel regions of the code'
	write(6,'(2x,a,i0)') 'Maximum number of threads = ', OMP_GET_MAX_THREADS()
	write(6,'(2x,a,i0)') 'Number of processors = ', OMP_GET_NUM_PROCS()
	...
	ALLOCATE( vc(M1-1:M2+1, N1-1:N2+1) )
	ALLOCATE( Fc(M1-1:M2+1, N1-1:N2+1) )
	ALLOCATE( Gc(M1-1:M2+1, N1-1:N2+1) )
	...
	DO WHILE ( t < tF_check )
		...
		DO external_iteration_count = 1, no_external_iterations
			...
			!$OMP PARALLEL  DEFAULT(none), SHARED(M1,M2,N1,N2,extra_size,main_chunk_size,Nthreads,  &
				q,u,vc,Fc,Gc, cfl,h_to_pm2,time_start,b1,b2,b3,b4,j1,j2,j3,j4,omp_iterations, &
				time_finish,sum_cputime, Qt_Atualizacoes_u) , &
				PRIVATE(tid,ic0,icF)

			tid = OMP_GET_THREAD_NUM();

			ic0 = M1 + tid*main_chunk_size;
			icF = ic0 + main_chunk_size;
			ic0 = ic0 + 1;

			! ****************************************************************** !
			!                computation of F values on the grid:                !
			! ****************************************************************** !
			! ******  F(i,j) for ic0 <= i <= icF + 1 and N1 <= j <= N2:

			Fc(ic0:icF+1,N1:N2) = ( (u(ic0:icF+1,N1:N2) - u(ic0-1:icF,N1:N2))**2 + &
				( (u(ic0-1:icF,N1+1:N2+1)+u(ic0:icF+1,N1+1:N2+1))-(u(ic0-1:icF,N1-1:N2-1)+u(ic0:icF+1,N1-1:N2-1)) )**2 /16 )**q / h_to_pm2;

			! ****************************************************************** !
			!                computation of G values on the grid:                !
			! ****************************************************************** !
			! ******  G(i,j) for ic0 <= i <= icF and N1 <= j <= N2 + 1:

			Gc(ic0:icF,N1:N2+1) = ( (u(ic0:icF,N1:N2+1) - u(ic0:icF,N1-1:N2))**2 + &
				( (u(ic0+1:icF+1,N1-1:N2)+u(ic0+1:icF+1,N1:N2+1))-(u(ic0-1:icF-1,N1-1:N2)+u(ic0-1:icF-1,N1:N2+1)) )**2 /16 )**q / h_to_pm2;


			! ****************************************************************** !
			!      computation of v = [ new u values at the new time level ]:    !
			! ****************************************************************** !
			! ******  v(i,j) for ic0 <= i <= icF and N1 <= j <= N2:

			vc(ic0:icF,N1:N2) = u(ic0:icF,N1:N2) + &
				cfl*( Fc(ic0+1:icF+1,N1:N2)*(u(ic0+1:icF+1,N1:N2)-u(ic0:icF,N1:N2)) - &
				Fc(ic0:icF,N1:N2)*(u(ic0:icF,N1:N2)-u(ic0-1:icF-1,N1:N2)) ) + &
				cfl*( Gc(ic0:icF,N1+1:N2+1)*(u(ic0:icF,N1+1:N2+1)-u(ic0:icF,N1:N2)) - &
				Gc(ic0:icF,N1:N2)*(u(ic0:icF,N1:N2)-u(ic0:icF,N1-1:N2-1)) );
			...
			if ( tid.EQ.0 ) then
				...
				vc(M1, N1:N2) = vc(M1+1, N1:N2);
				vc(M1-1, N1:N2) = vc(M1+1, N1:N2);
				vc(M1-1:icF,N1-1) = vc(M1-1:icF,N1);
				vc(M1-1:icF,N2+1) = vc(M1-1:icF,N2);
			...
			if ( tid.EQ.Nthreads-1 ) then
				...
				vc(M2+1, N1:N2) = vc(M2, N1:N2);
				vc(ic0:M2+1,N1-1) = vc(ic0:M2+1,N1);
				vc(ic0:M2+1,N2+1) = vc(ic0:M2+1,N2);
	...
	write(70,*)
	write(70,*) " No. of threads in OMP parallel regions: ", Nthreads
	write(70,*)
	...
	write(70,'(2x,a,i0)') "With extended block size = ", extra_size
	write(70,*)
	write(70,*) '***************************************************************************'
	write(70,*)
	write(70,*)
