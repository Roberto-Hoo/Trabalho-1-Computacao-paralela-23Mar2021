...
PROGRAM initialize4P

IMPLICIT NONE

LOGICAL, PARAMETER :: debug = .false.    ! <--- Escrever informacoes no modo debug
CHARACTER*1  char1
INTEGER, PARAMETER :: i64 = selected_int_kind(18)
INTEGER (i64)      :: Qt_Atualizacoes_u  ! <--- Quantidade total de atualizacoes de u

!  COMPUTATIONAL REGION will be: [-12, 12] x [-12, 12],  with h = 0.1:

INTEGER, PARAMETER ::  M1 = -100, M2 = 100   ! <--- x-grid is: x(M1),...,x(M2)
INTEGER, PARAMETER ::  N1 = -100, N2 = 100   ! <--- y-grid is: y(N1),...,y(N2)
...
REAL*8   d1, d2, d3, d4 !distância do pontos (x,y) aos cones

REAL*8   r12, r13, r14, r23, r24, r34,  r0
...
REAL*8   C1, C2, C3, C4, cc1, cc2, cc3 ,cc4
REAL*8   Rx, Ry
...
!  time_start = dble(DATE_start(7)) + dble(DATE_start(8))/1000D0;   ! <--- not appropriate
...
DO i = M1, M2
	...
	IF ( x(i) < x4 + h/100D0 ) i4 = i;    ! <--- i-index of 4rd singularity
ENDDO
...
DO j = N1, N2
	...
	IF ( y(j) < y4 + h/100D0 ) j4 = j;    ! <--- j-index of 4rd singularity
ENDDO
...
!x_min = M1*h; x_max = M2*h;    <--- already computed (above)
!y_min = N1*h; y_max = N2*h;    <--- already computed (above)
...
CLOSE(10)
if (debug) then
	print *, "Tecle uma tecla e depois Enter para finalizar..."
	read  *, char1;
end if
!--------------------------------------------------------------------------
!--------------------------------------------------------------------------
