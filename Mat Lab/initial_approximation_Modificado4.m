...
I1 = find( 2.50 <= abs(x) & abs(x) <= 3.50 );
J1 = find( -3.50 <= y & y <= 3.50 );

I2 = find( -2.50 < x & x < 2.50 );
J2 = find( 2.50 <= abs(y) & abs(y) <= 3.50 );

far_field_value_u0 = sum( sum( u0(I1,J1) ) )+sum( sum( u0(I2,J2) ) );
total =(length(I1)*length(J1)) + (length(I2)*length(J2));
far_field_value_u0 = far_field_value_u0/total;
b
